...
namespace TreeGenerator
{
    public class TreeBuilder : IDisposable
    {
                                        ...
                                        ImageFormat ImageType)
        {
            ...
            Color backColor = _BoxFillColor;
            Color foreColor = _FontColor;
            var node = tree.Find(StartFromNodeID);
            if (node != null)
            {
            ...
            XmlNode RootNode = GetXMLNode(StartFromNodeID, rootDescription, rootNote, backColor, foreColor);
        ...
        private void BuildTree(XmlNode oNode, int y)
        {
            ...
            //has children
            foreach (var childRow in tree.Parents(oNode.Attributes["nodeID"].InnerText))
            {
        ...
        */
        ...
        private XmlNode GetXMLNode(string nodeID, string nodeDescription, string nodeNote, Color backColor, Color foreColor)
        {
            ...
            XmlAttribute attBackColor = nodeTree.CreateAttribute("nodeBackColor");
            XmlAttribute attForeColor = nodeTree.CreateAttribute("nodeForeColor");
        ...
        private void DrawChart(XmlNode oNode)
        {
            ...
            SolidBrush drawBrush = new SolidBrush(Color.FromArgb(Convert.ToInt32(oNode.Attributes["nodeForeColor"].Value)));
            ...
            gr.FillRectangle(new SolidBrush(Color.FromArgb(Convert.ToInt32(oNode.Attributes["nodeBackColor"].Value))), currentRectangle);
            // Create string to draw.
            String drawString = Environment.NewLine + oNode.Attributes["nodeNote"].InnerText;// +
                //Environment.NewLine +
